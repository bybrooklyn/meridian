...
| Meridian UI | `ImplementedFoundation` / `Partial` / `Active` | MS-02 qualified retained UI core; `WP-UI-002` through `WP-UI-004` are locally implemented but unqualified because hosted CI could not allocate a runner, while `WP-UI-005` is active and `RG-UI-001` is open under non-promoting `WVR-UI-001`. Platform accessibility, renderer qualification, and the production shell remain open. |
