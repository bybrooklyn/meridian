...
| Build package scope | A single long-lived BLD package put MS-08 multi-node/remote-service work on the path to the MS-03 local-Cargo prerequisite, while ambient host tools could make project reproduction or updates implicit. | `WP-BLD-001` owns only the bounded MS-03 local-Cargo service seam. `WP-BLD-002` is a separate planned MS-08 package for managed external toolchains with exact project pins, multi-node result lineage, general artifact/cache policy, service-process/remote-worker supervision, and team profiles. Managed components install side-by-side with verified atomic activation and never silently change a project pin. |
...
| Animation and navigation | Cross-system behavior lacked dedicated authority. | Artus is the named `ANI` body-motion authority for semantic rigs, profiles, poses, retargeting, intent execution, contacts, negotiated Cairn coupling, and sequencing; `NAV` owns traversability/artifacts/queries; gameplay/frameworks and future Bearings layers retain decisions. MS-09 permits only facial profile/import/pose-layer foundation; advanced facial work remains post-1.0. |
...
| Meridian UI | `ImplementedFoundation` / `Partial` / `Active` | MS-02 qualified retained UI core; `WP-UI-002` through `WP-UI-005` are locally implemented but unqualified because hosted CI could not allocate runners. `RG-UI-001` is decided by ADR-0029. The winning replan retains one source model: `WP-UI-006` makes `UiDocument` the ergonomic, versioned authored source and is the sole active source-only package under `WVR-UI-001`; `WP-EDT-002` is suspended as `Partial`, then `WP-EDT-003` composes current Creator workspaces in MS-03 without simulating unfinished domain work. Local profile-bound raw-RGBA golden, controlled destruction/replay, and uncalibrated raw performance output is `Inconclusive` qualification evidence because source provenance is caller-declared; it does not complete the direct renderer, real platform screen-reader evidence, accessibility qualification, or the production shell. |
