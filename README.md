...
cargo run -p meridian-build --bin meridian-build -- --cargo-test-no-run --workspace . --source-checkpoint local-cargo-test-no-run --toolchain local --target host --artifact-store target/meridian-build-artifacts --cargo-output-root target -- -p meridian-core
~~~
