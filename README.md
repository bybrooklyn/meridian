...
cargo run -p meridian-build --bin meridian-build -- --cargo-test-no-run --workspace . --source-checkpoint local-cargo-test-no-run --toolchain local --target host --artifact-store target/meridian-build-artifacts --cargo-output-root target/meridian-build-cargo-test -- -p meridian-core --target-dir target/meridian-build-cargo-test
~~~
